...
func (r *ManageOrderRouter) InitManageOrderRouter(Router *gin.RouterGroup) {
	...
	{
		mallAdminUserWithoutRouter.POST("orders/name", mallOrderApi.GetMallUserAllOrder) // 根据传入的用户名 获得此用户所有的订单列表
		mallAdminUserWithoutRouter.GET("orders/name", mallOrderApi.GetMallUserAllOrder)  // 根据传入的用户名 获得此用户所有的订单列表
	}
}
